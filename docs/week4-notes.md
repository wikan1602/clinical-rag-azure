...
## Correction: the "intermittent DNS" issue above was actually a wrong endpoint value

While building the GitHub Actions pipeline later in Week 4 (see the CI/CD section of this file), the exact same `getaddrinfo failed` symptom reappeared, this time reproducing 100% of the time in CI instead of intermittently. Chasing it as a network problem (Docker `--network host`, explicit `--dns` flags, IPv6/`getaddrinfo` vs `gethostbyname` differences, running the candidate build without Docker at all) never fixed it, because it wasn't a network problem: `AZURE_OPENAI_ENDPOINT` in both `.env` and the GitHub secret was set to `https://openai-clinical-rag-12a3b.openai.azure.com/` — a hostname that has never existed. Confirmed via Google's and Cloudflare's public DoH resolvers, both returning NXDOMAIN with Azure's own authoritative nameserver as the referral; the real endpoint (`az cognitiveservices account show`) is `https://openai-clinical-rag.openai.azure.com/`, no suffix.

In hindsight, the "recurring intermittent DNS resolution failures" described above were almost certainly this same wrong hostname the whole time, only ever resolving when the local machine had a stale cached DNS entry for it from some earlier state — which would explain both the intermittency (cache expiry) and why it was never reliably reproducible. Fixed by correcting the endpoint value; the retry-with-backoff additions above remain in place as genuinely useful defense-in-depth, but they were never the actual fix for this particular failure mode.
